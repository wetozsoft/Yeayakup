...
if (is_array($_POST['chk'])) {
    $cnt_chk = count($_POST['chk']);
    for ($z = 0; $z < $cnt_chk; $z++) {
        $rmix   = (int)$_POST['rm_ix'][$_POST['chk'][$z]]; // 객실키
        $bkday  = (int)$_POST['bk_day'][$_POST['chk'][$z]]; // 예약일수
        $bkcnt_adult  = (int)$_POST['bk_cnt_adult'][$_POST['chk'][$z]]; // 예약인원
        if ($rmix) { 
...
<div class="st2-form">
    ...
    <table cellpadding="0" cellspacing="0" border="0" class="tbl_type">
        ...
        if ($cnt_room > 0) { 
            ...
            $add_stay_price = $arr_room[$z]['price'];
            $add_person_adult = $add_person_price = 0;
            if ($arr_room[$z]['bk_cnt_adult'] > $arr_room[$z]['rm_person_min']) { 
                $add_person_adult   = $arr_room[$z]['bk_cnt_adult'] - $arr_room[$z]['rm_person_min'];
                $add_person_price   = ($arr_room[$z]['rm_price_adult'] * $add_person_adult) * $arr_room[$z]['bk_day'];
            }

            $last_day = wz_get_addday($sch_day, $arr_room[$z]['bk_day']);
            ?>
            <input type="hidden" name="rm_ix[]"     value="<?php echo $arr_room[$z]['rm_ix'];?>" />
            <input type="hidden" name="bk_day[]"    value="<?php echo $arr_room[$z]['bk_day'];?>" />
            <input type="hidden" name="bk_cnt_adult[]"    value="<?php echo $arr_room[$z]['bk_cnt_adult'];?>" />
            <tr>
                <td><?php echo $arr_room[$z]['rm_subject'];?></td>
                <td><?php echo wz_get_hangul_date($sch_day).'('.get_yoil($sch_day).') ~ '.wz_get_hangul_date($last_day).'('.get_yoil($last_day).')';?></td>
                <td><?php echo $arr_room[$z]['bk_day'];?>박 <?php echo $arr_room[$z]['bk_day']+1;?>일</td>
                <td><?php echo $arr_room[$z]['bk_cnt_adult'];?>명</td>
                <td><?php echo number_format($add_stay_price);?> 원</td>
                <td><?php echo number_format($add_person_price);?> 원</td>
                <td><?php echo number_format($add_stay_price + $add_person_price);?> 원</td>
            </tr>
            <?php 
            $total_price += $add_stay_price + $add_person_price;
            }
        } 
        ?>
        </tbody>
    ...
    <h3>- 결제방법</h3>
    <input type="hidden" name="bk_payment" id="bk_payment" value="무통장" />
    <table cellpadding="0" cellspacing="0" border="0" class="tbl_type frm">
        <caption></caption>
        <colgroup>
            <col width="150px">
